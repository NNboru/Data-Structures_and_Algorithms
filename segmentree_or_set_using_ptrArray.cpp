...
Here count means set implementation, and lazy means seg-sum implementation.
*/
#include<iostream>
#include<cstdio>
#include<algorithm>
...
// for lazy operations- L & R are range for node nd, adds val from l to r. (all are pos not index)
...
//  L & R are range for node nd, return sum from l to r. (all are pos not index)
int query(int nd,int L,int R,int l,int r,int ex=0)
{
...
// for count operations. To increment cnt at position pos.
void add(int root,int l,int r,int pos)
{
	// root contains data for range l-r. 
	if(l==r)
	{
		// ptrs[root].cnt++; 	 // if multiple items can exist.(multiset tree)
		if(ptrs[root].cnt==0) 	// if multiple items cannot exist. (set tree)
...
void dfs(int root,int l,int r)
{
	if(ch[root]==0)
	{
		pr l sp<<r sp<<ptrs[root].val sp<<ptrs[root].extra en;
		ch[root]=1;
	}
...
main()
{
	...
	*/
	...
	pr "query(root,1,N,2,4) : " << query(root,1,N,2,4) en;
	pr "add(root,1,N,1,4,1)" en;
	add(root,1,N,1,4,1);
	dfs(root);
	pr "query(root,1,N,1,4): "<< query(root,1,N,1,4);
